...
async def get_curriculum(db:Session):
  ...
  try:
    sql_query = text("""
    SELECT s.CODPERIODO AS periodo, s.CODCURSO AS curso_id, s.CODDISC AS disciplina_id  FROM CEMGJB_128187_RM_DV.dbo.SDISCGRADE s WHERE s.CODGRADE ='20231' AND s.CODPERIODO != 0
    """)
