...
async def has_class_period(codcurso: str,codturno: int, periodoletivo_id: int, periodo_id: int, db:Session = Depends(get_db), api_key: str = Depends(get_api_key)):
    class_details = await course_services.has_class_period(codcurso=codcurso,codturno=codturno,periodoletivo_id=periodoletivo_id, periodo_id=periodo_id,db=db)
